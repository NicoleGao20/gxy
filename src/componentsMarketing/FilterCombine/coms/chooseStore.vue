<template>
  <div class="clearfix">
    ...
    <el-form-item :prop="prefixProp+'.column_value'" label-width="20px" style="display: inline-block;float:left;width: 400px;">
    ...
    <el-dialog
      ...
      append-to-body
      width="800px">
      <belle-choose-store :store-data="storeData" @onResulte="onResulte"/>
  ...
  methods: {
    ...
    confirmPickStoreHandler() {
      this.specItem.column_value = this.data.table.map((item) => {
        return item.orgCodeLong
      }).join(',')

      const list = []
      this.data.table.forEach(element => {
        list.push({ org_code_long: element.orgCodeLong, type: 'include' })
      })
      this.$request({
