<template>
  <div>
    <el-form-item label-width="0" style="display: inline-block;width: 150px;">
      <el-tooltip :content="conf.label_name" class="item" effect="light" placement="bottom">
    ...
    <el-form-item
      ...
      :rules="{
        required: (!conf.is_allow_null||conf.is_allow_null === 'N')? true: false, message: '请选择'+conf.label_name, trigger: 'change'
      }"
      ...
      <el-checkbox-group :disabled="conf.label_values.length === 1 || filterCombine.onlyRead" v-model="column_value" @input="changeSpecItem">
        <el-checkbox v-for="(option, index) in conf.label_values" :label="option.label_value_code" :key="index">{{ option.label_value || option.label_name }}</el-checkbox>
      </el-checkbox-group>
    </el-form-item>
  </div>
</template>
