<template>
  <div class="page-container-gap">
    ...
    <main>
      <el-button type="primary" @click="labelModalVisble = true" >新增</el-button>
      <el-table v-loading="loading" stripe :data="list" :max-height="500" style="margin-top: 10px;">
        <el-table-column label="序号">
          <template slot-scope="scope"><span>{{ (scope.$index+1) }}</span></template>
        </el-table-column>
        <el-table-column label="标签名称" prop="label_name" width="160">
        ...
        <el-table-column label="标签类型" prop="label_type">
          <template slot-scope="scope">
        ...
        <el-table-column label="标签属性" prop="label_attribute">
          <template slot-scope="scope">
            {{ getLabelByValue(scope.row.label_attribute, labelAttributes) }}
          </template>
        </el-table-column>
        <el-table-column label="应用品牌" prop="brand_info" width="140">
          <template slot-scope="scope">
        ...
        <el-table-column label="创建人" prop="create_user" width="100">
          <template slot-scope="scope">
        ...
        <el-table-column label="修改人" prop="update_user" width="100">
          <template slot-scope="scope">
            <human-info :num="scope.row.update_user_code">{{ scope.row.update_user }}</human-info>
          </template>
        </el-table-column>
        <el-table-column label="最新修改时间" width="100" prop="last_modify_time"/>
    ...
    <el-dialog :visible.sync="labelModalVisble" title="创建标签">
      <div>
        ...
        <div class="label-tip">
          <div class="label-item-tip">手工标签：会员手动打标，可用于手动打标、营销活动打标；</div>
          <div class="label-item-tip">规则标签：通过基础属性筛选出的标签；</div>
          <div class="label-item-tip">复合标签：通过手工标签/基础标签/模型标签筛选出的标签；</div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      actionRouteMap: {
        manual_adds: 'manualLabelAdds',
        rule_adds: 'ruleLabelAdds',
        complex_adds: 'complexLabelAdds',
        attribute_adds: 'attrLabelAdds',
        model_adds: 'rfmLabelAdds',

        manual_edits: 'manualLabelEdits',
        rule_edits: 'ruleLabelEdits',
        complex_edits: 'complexLabelEdits',
        attribute_edits: 'attrLabelEdits',
        model_edits: 'rfmLabelEdits'
      },
      brand_list: [],
      createFormRules: {
        label_type: [
  ...
  methods: {
    ...
    add(row) {
      this.$router.push({
        name: this.actionRouteMap[row.label_type + '_adds']
      })
    },
    view(row) {
      if (row.create_type && row.id) {
        this.$router.push({
          name: this.actionRouteMap[row.label_type + '_edits'],
          query: {
            id: row.id,
    ...
    switchType(row) {
      this.$msgbox({
        title: '确认消息',
        message: `确定将此标签转换为${row.label_attribute === 'common' ? '个性' : '通用'}标签`,
        showClose: true,
