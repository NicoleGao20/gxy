          ...
          <el-row :gutter="20" style="margin-top: 35px;">
            ...
            <el-col :span="8">
              <el-form-item label="标签名称" prop="label_name">
                <el-input v-model.trim="form.label_name" :disabled="(actionType === 'edit' && form.brand_info.length) || $route.query.view" maxlength="50" clearable/>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="有效开始日期" prop="valid_time_begin">
                <el-date-picker
                :disabled="(actionType === 'edit' && form.brand_info.length) || $route.query.view"
            ...
            <el-col :span="24">
              <el-form-item label="标签说明" prop="label_desc">
                <el-input    :disabled="(actionType === 'edit' && form.brand_info.length) || $route.query.view" v-model="form.label_desc" maxlength="400" placeholder="标签说明,最多可以填400字"/>
              </el-form-item>
            </el-col>
          </el-row>
        </div>
...
规则标签 新增时只有筛选器
*/
import { addManualLabel, editManualLabel, getManualLabel } from '@/api/labelPool'
  ...
  methods: {
    init() {
      if (!this.$route.query.id) {
        ...
        getManualLabel({ id: this.$route.query.id, create_type: 'manual' }).then((res) => {
          var data = res.data
    ...
    confirmHandler() {
      ...
      this.$refs['form'].validate((valid) => {
        ...
        if (!this.$route.query.id) {
          addManualLabel(params).then((res) => {
          ...
          editManualLabel(params).then((res) => {
            this.actionLoading = false
            this.$message({
              message: '编辑成功!',
              type: 'success',
