<template>
  <div class="page-container-gap">
          ...
          <el-col :span="6">
            <el-form-item label="数据集">
              <el-select
                ...
                style="width: 100%"
                filterable
                @focus="getlistTableNameShow"
              >
                <el-option
                  v-for="(item, index) in listTableNameShow"
                  :key="index"
                  :label="item.table_name_show"
                  :value="item.table_name_show"
                />
              </el-select>
            </el-form-item>
          </el-col>
    ...
    <div style="padding: 10px; background: #fff">
      <el-table v-loading="loading" stripe :data="rows" max-height="550" border>
        <el-table-column label="类型" prop="table_type">
          <template slot-scope="scope">
            <span v-if="scope.row.table_type === 'normal'"> 标签数据 </span>
            <span v-if="scope.row.table_type === 'aggregation'">
              行为数据
            </span>
          </template>
        </el-table-column>
        <el-table-column  label="数据集" prop="table_name_show" />
        <el-table-column label="更新频次" prop="update_frequency">
          <template slot-scope="scope">
            <span v-if="scope.row.update_frequency === 'realtime'"> 实时 </span>
            <span v-if="scope.row.update_frequency === 't_1'"> 每天 </span>
          </template>
        </el-table-column>
        <el-table-column label="创建人" prop="create_user" />
        <el-table-column label="修改人" prop="update_user" />
        <el-table-column label="逻辑表名" prop="table_name" />
        <el-table-column label="描述" prop="table_name_desc" />
...
export default {
  ...
  methods: {
    ...
    getlistTableNameShow() {
      ...
      const params = { table_type: "aggregation"};
