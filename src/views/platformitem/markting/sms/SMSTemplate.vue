<template>
  <div class="page-container-gap">
    ...
    <el-button type="primary" class="listAddBtnStyle" @click="$router.push({name:'newSMSTemplate'})">新增</el-button>
    <el-table
    :data="tableData"
    stripe
    border
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        ...
        <el-button type="text" @click="deleteTemplate(scope.row.id,scope.row.template_code)" class="deleteFontColor" v-if="scope.row.status != 0">删除</el-button>
      </template>
    </el-table-column>
    </el-table>
    <el-pagination
...
<script>
import { queryTemplateByPage, deleteTemplate } from "@/api/markting/SMSFunction"
export default {
  mounted(){
  ...
  methods:{
    getTableData(pageNum,pageSize){
      ...
      if(this.form.type || this.form.type === 0) obj.template_type = this.form.type
      if(this.form.smsType || this.form.smsType === 0) obj.type = this.form.smsType
