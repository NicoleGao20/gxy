<template>
  <div class="wechat-template-message-container page-container-gap">
    ...
    <main class="tableMainContent">
      <router-link :to="{name:'newWechatTemplateMessage'}"><el-button type="primary" class="mb10">新增</el-button></router-link>
      <el-table
        v-loading="loading"
        :data="tableData"
      ...
      >
        ...
        <el-table-column prop="trigger_type" label="触发类型" width="130" />
        ...
        <el-table-column prop="msg_send_status" label="发送状态" width="110" />
        <el-table-column prop="create_time" label="创建时间" width="140" />
        <el-table-column prop="create_user" label="创建人" width="110" />
        <el-table-column prop="update_user" label="修改人" width="110" />
...
<script>
import { queryWeChatTplMsg, stopWxTplmsg } from '@/api/markting/wechat'
...
export default {
  ...
  methods: {
    ...
    handleView(row) {
      this.$router.push({ name: 'viewWechatTemplateMessage', query: { id: row.id }})
    },
    handleEdit(row) {
      this.$router.push({ name: 'editWechatTemplateMessage', query: { id: row.id }})
    },
    handleViewLog(row) {
      this.$router.push({ name: 'wechatTemplateMessageLog', query: { id: row.id }})
    },
    showStopDialog(row) {
      this.row = row
    ...
    handleStop() {
      ...
      stopWxTplmsg({ id: this.row.id, msg_send_status: '已停止', xxl_id: this.row.xxl_id }).then(res => {
        if (res && res.data) {
...
.wechat-template-message-container {
  padding: 0 15px;
  header,
  main {
    width: 100%;
